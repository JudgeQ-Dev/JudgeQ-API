...
export class JudgeClientController {
  ...
  async addJudgeClient(
    ...
    const judgeClient = await this.judgeClientService.addJudgeClient(
      request.name,
      request.allowedHosts,
    );

    return {
      judgeClient: await this.judgeClientService.getJudgeClientInfo(
        judgeClient,
        true,
      ),
    };
  ...
  async deleteJudgeClient(
    ...
    const judgeClient = await this.judgeClientService.findJudgeClientById(
      request.id,
    );
    if (!judgeClient)
      return {
  ...
  async listJudgeClients(
    ...
    return {
      judgeClients: await Promise.all(
        judgeClients.map(
          async (judgeClient) =>
            await this.judgeClientService.getJudgeClientInfo(
              judgeClient,
