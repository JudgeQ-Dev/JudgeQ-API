...
const xdomainScript = minifyJs(
  fs.readFileSync(require.resolve("xdomain/dist/xdomain.min"), "utf-8"),
).then(({ code }) => code);

// streamSaver
const streamSaverMitmHtml = (async () => {
  const $ = cheerio.load(
    fs.readFileSync(require.resolve("streamsaver/mitm.html"), "utf-8"),
  );
  const script = $("script").html();
  const minifiedScript = (await minifyJs(script)).code;
  return `<script>${minifiedScript}</script>`;
})();
const streamSaverSwScript = minifyJs(
  fs.readFileSync(require.resolve("streamsaver/sw.js"), "utf-8"),
).then(({ code }) => code);
...
export class CorsController {
  ...
  @ApiOperation({
    summary:
      "Get the proxy page for streamsaver (used for download-as-archive).",
  })
  ...
  @ApiOperation({
    summary:
      "Get the service worker script for streamsaver (used for download-as-archive).",
  })
