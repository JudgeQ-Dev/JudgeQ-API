...
export function validateMetaAndSubtasks(
  ...
  const validateMemoryLimit = (
    ...
    if (
      options.hardMemoryLimit != null &&
      memoryLimit > options.hardMemoryLimit
    )
      throw [
  ...
  if (options.enableFileIo && judgeInfo.fileIo) {
    if (
      typeof judgeInfo.fileIo.inputFilename !== "string" ||
      !isValidFilename(judgeInfo.fileIo.inputFilename)
    )
      throw ["INVALID_FILEIO_FILENAME", judgeInfo.fileIo.inputFilename];
    if (
      typeof judgeInfo.fileIo.outputFilename !== "string" ||
      !isValidFilename(judgeInfo.fileIo.outputFilename)
    )
  ...
  (judgeInfo.subtasks || []).forEach((subtask, i) => {
    ...
    if (
      points != null &&
      (typeof points !== "number" || points < 0 || points > 100)
    )
    ...
    if (Array.isArray(dependencies)) {
      dependencies.forEach((dependency) => {
        if (
          ...
          dependency < 0 ||
          dependency >= judgeInfo.subtasks.length
        )
    ...
    if (!Array.isArray(testcases) || testcases.length === 0)
      throw ["SUBTASK_HAS_NO_TESTCASES", i + 1];
    ...
    testcases.forEach((testcase, j) => {
      ...
      if (options.enableUserOutputFilename) {
        ...
        if (duplicateIndex !== -1) {
          const [, duplicateSubtaskIndex, duplicateTestcaseIndex] =
            userOutputFilenames[duplicateIndex];
          throw [
      ...
      if (options.enableTimeMemoryLimit) {
        ...
        if (memoryLimit != null)
          validateMemoryLimit(memoryLimit, "TESTCASE", i, j);
      } else {
    ...
    // eslint-disable-next-line @typescript-eslint/no-shadow
    const sum = testcases.reduce(
      (s, { points }) => (points ? s + points : s),
      0,
    );
    if (sum > 100) {
  ...
  if (
    ...
    judgeInfo.subtasks &&
    judgeInfo.subtasks.reduce(
      (count, subtask) => count + subtask.testcases.length,
      0,
    ) > options.testcaseLimit
  )
