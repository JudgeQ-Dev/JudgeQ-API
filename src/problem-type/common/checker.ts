...
export function validateChecker(
  ...
  switch (judgeInfo.checker.type) {
    ...
    case "floats":
      if (
        !(
          Number.isSafeInteger(judgeInfo.checker.precision) &&
          judgeInfo.checker.precision > 0
        )
      )
    ...
    case "custom": {
      ...
      if (
        !["testlib", "legacy", "lemon", "hustoj", "qduoj", "domjudge"].includes(
          checker.interface,
        )
      )
        throw ["INVALID_CHECKER_INTERFACE"];
      if (!Object.values(CodeLanguage).includes(checker.language))
        throw ["INVALID_CHECKER_LANGUAGE"];
      if (!testData.some((file) => file.filename === checker.filename))
        throw ["NO_SUCH_CHECKER_FILE", checker.filename];
      if (
        !options.validateCompileAndRunOptions(
          checker.language,
