...
export class ProblemController {
  ...
  async queryProblemSet(
    ...
    if (
      (request.ownerId || request.nonpublic) &&
      !hasPrivilege &&
      (!currentUser || currentUser.id !== request.ownerId)
    ) {
      return {
    ...
    const filterTags = !request.tagIds
      ? null
      : (
          await this.problemService.findProblemTagsByExistingIds(request.tagIds)
        ).filter((tag) => tag);
    const filterOwner = !request.ownerId
      ? null
      : await this.userService.findUserById(request.ownerId);
    ...
    if (request.keyword && request.keywordMatchesId) {
      const matchId =
        request.keyword.substr(0, 1).toUpperCase() === "P"
          ? Number(request.keyword.slice(1)) || 0
          : 0;
      const matchDisplayId = Number(request.keyword) || 0;
      if (
        !problems.some(
          (problem) =>
            problem.id === matchId || problem.displayId === matchDisplayId,
        )
      ) {
    ...
    return {
      ...
      result: await Promise.all(
        problems.map(async (problem) => {
          ...
          const problemTags =
            !request.titleOnly &&
            (await this.problemService.getProblemTagsByProblem(problem));
          return {
            ...
            tags:
              ...
              (await Promise.all(
                problemTags.map((problemTag) =>
                  this.problemService.getProblemTagLocalized(
                    problemTag,
                    request.locale,
                  ),
                ),
            ...
            submission:
              !request.titleOnly &&
              currentUser &&
              (acceptedSubmissions.get(problem.id) ||
                nonAcceptedSubmissions.get(problem.id)),
          };
        }),
      ),
      permissions: !request.titleOnly && {
        createProblem: await this.problemService.userHasCreateProblemPermission(
          currentUser,
        ),
      ...
      filterOwner:
        !request.titleOnly &&
        filterOwner &&
        (await this.userService.getUserMeta(filterOwner, currentUser)),
    };
  }
  ...
  async createProblem(
    ...
    if (
      !(await this.problemService.userHasCreateProblemPermission(currentUser))
    )
      return {
        error: CreateProblemResponseError.PERMISSION_DENIED,
      };

    const problemTags = await this.problemService.findProblemTagsByExistingIds(
      request.statement.problemTagIds,
    );
  ...
  async updateStatement(
    ...
    const oldStatement = {
      localizedContents:
        await this.problemService.getProblemAllLocalizedContents(problem),
      problemTagIds: await this.problemService.getProblemTagIdsByProblem(
        problem,
      ),
  ...
  async getProblem(
    ...
    if (request.id)
      problem = await this.problemService.findProblemById(request.id);
    ...
    if (request.localizedContentsOfLocale != null) {
      const resultLocale = problem.locales.includes(
      ...
      if (!request.localizedContentsTitleOnly) {
        promises.push(
          this.problemService
            ...
            .then(
              (contentSections) =>
                (result.localizedContentsOfLocale.contentSections =
                  contentSections),
            ),
        );
      }
        ...
        this.problemService
          ...
          .then(
            (localizedContentsOfAllLocales) =>
              (result.localizedContentsOfAllLocales =
                localizedContentsOfAllLocales),
          ),
      );
    }
    ...
    if (request.tagsOfAllLocales) {
      promises.push(
        this.problemService
            ...
            Promise.all(
              problemTags.map(async (problemTag) => {
                const localizedNames =
                  await this.problemService.getProblemTagAllLocalizedNames(
                    problemTag,
                  );
                return {
                  id: problemTag.id,
                  color: problemTag.color,
                  localizedNames: Object.entries(localizedNames).map(
                    ([locale, name]) => ({
          ...
          .then(
            (tagsOfAllLocales) => (result.tagsOfAllLocales = tagsOfAllLocales),
          ),
      );
    }
    ...
    if (request.samples) {
      promises.push(
        this.problemService
          .getProblemSamples(problem)
          .then((samples) => (result.samples = samples)),
      );
    }
    ...
    if (request.additionalFiles) {
      promises.push(
        this.problemService
          ...
          .then(
            (additionalFiles) => (result.additionalFiles = additionalFiles),
          ),
      );
    }
    ...
    if (request.discussionCount)
      promises.push(
        this.discussionService
          ...
          .then(
            (discussionCount) => (result.discussionCount = discussionCount),
          ),
      );
    ...
    if (request.permissionOfCurrentUser) {
      promises.push(
        this.problemService
          ...
          .then(
            (permissions) => (result.permissionOfCurrentUser = permissions),
          ),
      );
    }
      ...
      promises.push(
        (async () => {
          const userPermissions =
            await this.problemService.getProblemPermissions(problem);
    ...
    if (request.contestId)
      contest = await this.contestService.findContestById(request.contestId);
        ...
        (async () => {
          ...
          if (currentUser) {
            if (!contest) ok = true;
              ...
              (await this.contestService.getContestStatus(contest)) ===
              ContestStatusType.Finished
            )
              ok = true;
          }
          ...
          if (ok) {
            ...
            const lastAcceptedSubmission =
              lastSubmission &&
              lastSubmission.status === SubmissionStatus.Accepted
                ? lastSubmission
                : (
                    await this.submissionService.getUserLatestSubmissionByProblems(
                      currentUser,
            ...
            result.lastSubmission = {
              ...
              lastSubmissionContent:
                lastSubmission &&
                (
                  await this.submissionService.getSubmissionDetail(
                    lastSubmission,
                  )
                ).content,
            };
      ...
      async (problem) => {
        ...
        if (
          !(await this.problemService.userHasPermission(
            currentUser,
            problem,
            ProblemPermissionType.ManagePermission,
          ))
        )
          return {
            error: SetProblemPermissionsResponseError.PERMISSION_DENIED,
        ...
        const oldPermissions =
          await this.problemService.getProblemPermissionsWithId(problem);
        ...
        await this.auditService.log(
          "problem.set_permissions",
          AuditLogObjectType.Problem,
          problem.id,
          {
            oldPermissions: {
              userPermissions: oldPermissions.map(
  ...
  async setProblemDisplayId(
    ...
    if (
      !(await this.problemService.userHasPermission(
        currentUser,
        problem,
        ProblemPermissionType.ManagePublicness,
      ))
    )
      return {
  ...
  @ApiOperation({
    summary:
      "Upload or add an existing file to a problem as its testdata or additional file.",
    description: "Recaptcha required.",
  })
  ...
  async removeProblemFiles(
    ...
    await this.problemService.removeProblemFiles(
      problem,
      request.type,
      request.filenames,
    );
  ...
  async downloadProblemFiles(
    ...
    const problemFiles = await this.problemService.listProblemFiles(
      problem,
      request.type,
    );
    const downloadList = problemFiles.filter(
      (problemFile) =>
        request.filenameList.length === 0 ||
        request.filenameList.includes(problemFile.filename),
    );
  ...
  async renameProblemFile(
    ...
    if (
      !(await this.problemService.renameProblemFile(
        problem,
        request.type,
        request.filename,
        request.newFilename,
  ...
  async getProblemTagDetail(
    ...
    const localizedNames =
      await this.problemService.getProblemTagAllLocalizedNames(problemTag);
    return {
      id: problemTag.id,
      color: problemTag.color,
  ...
  async updateProblemTag(
    ...
    const oldLocalizedNames =
      await this.problemService.getProblemTagAllLocalizedNames(problemTag);
    const oldColor = problemTag.color;

    const localizedNameTuples: [Locale, string][] = request.localizedNames.map(
  ...
  async changeProblemType(
    ...
    return await this.problemService.lockProblemById<ChangeProblemTypeResponseDto>(
      ...
      async (problem) => {
        ...
        const [oldJudgeInfo] = await this.problemService.getProblemJudgeInfo(
          problem,
        );

        if (
          !(await this.problemService.changeProblemType(problem, request.type))
        )
          return {
