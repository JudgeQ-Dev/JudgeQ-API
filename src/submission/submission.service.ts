...
import {
  ...
  SubmissionEventType,
} from "./submission-progress.service";
...
function makeSubmissionPriority(
  ...
  const k =
    (userOccupiedTimeRecently - avgEveryUsersOccupiedTimeRecently) /
...
export class SubmissionService
  implements JudgeTaskService<SubmissionProgress, SubmissionTaskExtraInfo>
{
  constructor(
    @InjectConnection()
    ...
    this.auditService.registerObjectTypeQueryHandler(
      ...
      async (submissionId) => {
        ...
        return !submission
          ? null
          : await this.getSubmissionBasicMeta(submission);
      },
    );
  }
  ...
  async querySubmissions(
    ...
    const [hasSmallerId, hasLargerId] = await Promise.all([
      ...
      queryBuilderWithoutPagination
        .clone()
        .andWhere("id > :largestId", { largestId })
        .take(1)
        .getCount(),
    ]);
  ...
  > {
    const problemTypeService = this.problemTypeFactoryService.type(
      problem.type,
    );
    ...
    const validationError = await problemTypeService.validateSubmissionContent(
      content,
    );
    if (validationError && validationError.length > 0)
      return [validationError, null, null];

    const [fileUploadErrorOrRequest, submission] =
      ...
      >("READ COMMITTED", async (transactionalEntityManager) => {
        ...
        if (problemTypeService.shouldUploadAnswerFile()) {
          const result = await this.fileService.processUploadRequest(
            uploadInfo,
            (size) =>
              size <= this.configService.config.resourceLimit.submissionFileSize
                ? null
                : "FILE_TOO_LARGE",
        ...
        const pair =
          await problemTypeService.getCodeLanguageAndAnswerSizeFromSubmissionContentAndFile(
            content,
            file,
          );
    ...
    if (submission) {
      ...
      try {
        ...
        logger.error(
          `Failed to start judge for submission ${submission.id}: ${e}`,
        );
      }
  ...
  async getSubmissionDetail(
    submission: SubmissionEntity,
  ): Promise<SubmissionDetailEntity> {
  ...
  async getUserRecentlySubmissionCountPerDay(
    ...
    const getConvertTimezoneExpression = (valueToBeConverted: string) =>
      timezone === localTimezone
        ? valueToBeConverted
        : `CONVERT_TZ(${valueToBeConverted}, :localTimezone, :timezone)`;
    const queryResult: {
      submitDate: Date;
      count: string;
      ...
      .createQueryBuilder()
      .select(
        `DATE(${getConvertTimezoneExpression("submitTime")})`,
        "submitDate",
      )
      .addSelect("COUNT(*)", "count")
      .where("submitterId = :submitterId", { submitterId: user.id })
      .andWhere(
        `submitTime >= DATE_SUB(${getConvertTimezoneExpression(
          ":now",
        )}, INTERVAL :offsetDays DAY)`,
        {
          now,
          offsetDays: days - 1,
        },
    ...
    if (queryResult.length === 1 && queryResult[0].submitDate === null)
    ...
    const map = new Map(
      queryResult.map((row) => [
        ...
        // 3. Get its timestamp
        moment
          .tz(moment(row.submitDate).format("YYYY-MM-DD"), timezone)
          .valueOf(),
        Number(row.count),
      ]),
    );
    ...
    const result = [...new Array(days).keys()].map(
      (i) => map.get(startDate.clone().add(i, "day").valueOf()) || 0,
    );
  ...
  async getSubmissionCountFilterByStatus(
    ...
    return new Map(
      queryResult.map((result) => [result.problemId, parseInt(result.count)]),
    );
  }
  ...
  async getAllRecentlySubmissionCountPerDay(
    ...
    queryBuilder.groupBy("submitDate").setParameters({
      localTimezone,
      timezone,
    });
  ...
  async judgeSubmission(
    ...
    const [
      ...
      if (
        !this.configService.config.preference.serverSideOnly.dynamicTaskPriority
      )
        return [0, 0, 0, 0];
      ...
      const selectTotalOccupiedTimeRecently = (
        qb: QueryBuilder<SubmissionEntity>,
      ) =>
        qb
          .select("SUM(totalOccupiedTime)", "total")
          .andWhere("submitTime >= NOW() - INTERVAL 15 MINUTE");
      ...
      const [
        ...
        // userOccupiedTimeRecently
        selectTotalOccupiedTimeRecently(
          this.submissionRepository.createQueryBuilder(),
        )
          .where({
            submitterId: submission.submitterId,
          })
  ...
  async cancelSubmission(submission: SubmissionEntity): Promise<void> {
    // eslint-disable-next-line @typescript-eslint/no-shadow
    const canceled = await this.lockSubmission(
      submission,
      true,
      async (submission) => {
  ...
  async deleteSubmission(submission: SubmissionEntity): Promise<void> {
    ...
    await this.lockSubmission(submission, true, async (submission) => {
      ...
      await this.connection.transaction(
        ...
        async (transactionalEntityManager) => {
          ...
          if (submissionDetail.fileUuid)
            deleteFileActually = await this.fileService.deleteFile(
      ...
      await this.submissionStatisticsService.onSubmissionUpdated(
        submission,
        null,
      );
      if (submission.status === SubmissionStatus.Accepted) {
        await this.problemService.updateProblemStatistics(
  ...
  private async onSubmissionFinished(
    ...
    logger.log(
      `Submission ${submission.id} finished with status ${submission.status}`,
    );
  ...
  async onTaskProgress(
    ...
    if (!submission) {
      logger.warn(
        `Invalid task Id ${taskId} of task progress, maybe there's a too-early rejudge?`,
      );
      return false;
    }
  ...
  async getTaskToBeSentToJudgeByTaskId(
    taskId: string,
    priotity: number,
    ...
    try {
      ...
      const [preprocessedJudgeInfo] =
        await this.problemService.getProblemPreprocessedJudgeInfo(problem);
      const testData = await this.problemService.getProblemFiles(
        problem,
  ...
  async lockSubmission<T>(
    ...
    if (lockProblem) {
      return await this.problemService.lockProblemById(
        submission.problemId,
        "Read",
        async (problem) => {
          ...
          return await this.lockService.lock(
            ...
            async () =>
              await callback(
                await this.findSubmissionById(submission.id),
                problem,
              ),
          );
        },
  ...
  async getUserProblemAcceptedSubmissionCount(
    userId: number,
    problemId: number,
  ): Promise<number> {
  ...
  async getUserLatestSubmissionByProblems(
    ...
    return new Map(
      submissions.map((submission) => [submission.problemId, submission]),
    );
  }
